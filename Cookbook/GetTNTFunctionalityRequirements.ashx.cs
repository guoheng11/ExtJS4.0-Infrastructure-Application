...
{
    /// <summary>
    /// Summary description for GetTNTFunctionalityRequirements
    /// </summary>
    public class GetTNTFunctionalityRequirements : DatabaseHandler
    {
        public override PagedData ProcessRequest(HttpContext context, CookDBDataContext db)
        {
            IQueryable<TNTFunctionalityReq> q = db.TNTFunctionalityReqs;
            ...
            else
            {
                ...
                return new PagedData("GetTNTFunctionalityRequirements expects a project_id");
            }
            ...
            switch (context.Request.RequestType)
            {
                case "GET":
                    {
                        return new PagedData(q.Select(a => new { a.tnt_functionality_req_id, a.project_id, a.@new, a.call_volume, a.notes }));
                    }
                case "POST":
                    {
                        if (blob["rows"].GetType() == typeof(JObject))
                        {
                            ...
                            TNTFunctionalityReq record = new TNTFunctionalityReq();
                            ...
                            db.TNTFunctionalityReqs.InsertOnSubmit(record);
                            db.SubmitChanges();
                            ...
                            newLog.description = "New TNT Functionality requirement added";
                        ...
                        List<TNTFunctionalityReq> list = new List<TNTFunctionalityReq>();
                ...
                case "PUT":
                    {
                        if (blob["rows"].GetType() == typeof(JObject))
                        {
                            ...
                            string logBuilder = "";
                            string intro = "Existing TNT Functionality record modified: ";
                            
                            TNTFunctionalityReq record = db.TNTFunctionalityReqs.Single(a => a.tnt_functionality_req_id.Equals((int)obj["tnt_functionality_req_id"]));
                            //record.project_id = int.Parse(filter);
                        ...
                        for (int j = 0; j < objs.Count; j++)
                        {
                            TNTFunctionalityReq record = db.TNTFunctionalityReqs.Single(a => a.tnt_functionality_req_id.Equals((int)objs[j]["tnt_functionality_req_id"]));
                            //record.project_id = int.Parse(filter);
                            record.@new = (bool)objs[j]["new"];
                            record.call_volume = (string)objs[j]["call_volume"];
                            record.notes = (string)objs[j]["notes"];
                ...
                case "DELETE":
                    {
                        if (blob["rows"].GetType() == typeof(JObject))
                        {
                            ...
                            db.TNTFunctionalityReqs.DeleteOnSubmit(record);
                            ...
                            newLog.tab = "Requirements";
                            newLog.user_name = username;
                            newLog.description = "Existing TNT Functionality Requirement deleted: " + logbuilder;
                            if (!db.ChangeLogs.Contains(newLog))
                            {
                                db.ChangeLogs.InsertOnSubmit(newLog);
                                db.SubmitChanges();
                        ...
                        db.SubmitChanges();
                        return new PagedData("TNTFunctionalityReq deleted");
                    }
                default:
