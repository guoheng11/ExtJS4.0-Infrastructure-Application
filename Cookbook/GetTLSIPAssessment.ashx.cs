...
namespace Cookbook
{
    public class GetTLSIPAssessment : DatabaseHandler
    {
                ...
                case "GET":
                    {
                        ...
                        {
                            return new PagedData("GetTLSIPAssessment expects a project_id");
                        }
                    }
                case "POST":
                    {
                        if (blob["rows"].GetType() == typeof(JObject))
                        {
                            ...
                            {
                                record.assessment_type_id = db.AssessmentTypes.Single(a => a.type == "Other TLS_IP").assessment_type_id;
                            }
                ...
                case "DELETE":
                    {
                        if (blob["rows"].GetType() == typeof(JObject))
                        {
                            ...
                            db.SubmitChanges();
                            return new PagedData("TLSIP Assessment Rec Deleted");
                        }
                        else
                        {
                            JArray objs = (JArray)blob["rows"];
                            ...
                            return new PagedData("TLSIP Assessment Recs Deleted");
                        }
                    }
                default:
