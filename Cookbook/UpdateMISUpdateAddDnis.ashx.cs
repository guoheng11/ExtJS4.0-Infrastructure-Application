...
{
    /// <summary>
    /// Summary description for UpdateMISUpdateAddDnis
    /// </summary>
    public class UpdateMISUpdateAddDnis : DatabaseHandler
    {
                ...
                case "GET":
                    {
                        ...
                        if (!isNull(filter))
                        {
                            q = q.Where(a => a.mis_update_id.Equals(int.Parse(filter)) && a.route_to.Length > 1);
                        }

                        return new PagedData(q.Select(a => new { a.mis_updatednis_id, a.mis_update_id, a.dnis, a.route_to, a.platform, a.description, a.effective_date }));
                    }
                case "POST":
                    {
                        if (blob["rows"].GetType() == typeof(JObject))
                        {
                            ...
                            return new PagedData(new { record.mis_updatednis_id, record.mis_update_id, record.dnis, record.route_to, record.platform, record.description, record.effective_date });
                        }
                        ...
                        for (int j = 0; j < objs.Count; j++)
                        {
                            ...
                            list.Add(new { record.mis_updatednis_id, record.mis_update_id, record.dnis, record.route_to, record.platform, record.description, record.effective_date });
                        }
